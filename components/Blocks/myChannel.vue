<template>
  <div class="my-channel">
    ...
    <div class="myResourceCards">
      <div class="myResourceCard_container" v-for="(value, index) in userStore?.getProfileChannels" :key="index">
      ...
      <div class="message" v-if="!userStore?.getProfileChannels?.length">
        <h2 class="page-subtitle">Тут пока ничего нет</h2>
      </div>
      <Dialog v-model:visible="visibleEdit" :draggable="false" header="Редактировать канал" modal :style="{ width: '50vw' }">
        <div>
      ...
      <Dialog v-model:visible="visibleDel" header="Удалить канал" modal :style="{ width: '50vw' }">
        <h2>Вы уверены, что хотите удалить канал?</h2>
        <template #footer>
          <Button label="Удалить" icon="pi pi-check" @click="deleteChannelFunction()" />
...
async function deleteChannelFunction() {
  const data = new FormData();
  data.append('ids', selectedItem.value.id);

  await resourceStore.deleteResource(data);
  await userStore.requestProfile(userStore.data.id);
...
//   let myChannel = [];
//   for (let i = 0; i < userStore.profile.resources.length; ++i) {
//     if (userStore.profile?.user.resources[i].restypeId === 3) {
