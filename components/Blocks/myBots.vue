<template>
  <div class="my-bots">
    ...
    <div class="myResourceCards">
      <div class="myResourceCard_container" v-for="(value, index) in userStore?.getProfileBots" :key="index">
      ...
      <div class="message" v-if="!userStore?.getProfileBots?.length">
        <h2 class="page-subtitle">Тут пока ничего нет</h2>
      </div>
      <Dialog v-model:visible="visibleEdit" :draggable="false" header="Редактировать бота" modal :style="{ width: '50vw' }">
        <div>
      ...
      <Dialog v-model:visible="visibleDel" header="Удалить бота" modal :style="{ width: '50vw' }">
        <h2>Вы уверены, что хотите удалить бота?</h2>
        <template #footer>
          <Button label="Удалить" icon="pi pi-check" @click="deleteBotsFunction()" />
...
async function openEditModal(value) {
  ...
  // if(!isNationality(selectedItem?.value?.coverageId)) {
  //   selectedItem.value.countryId = getCountryByCoverage(selectedItem.value.countryId)[0]?.id;
  // }
  await regionStore.get(selectedItem.value.countryId);
...
function GetMyResource() {
  ...
  for (let i = 0; i < userStore.profile?.user.resources.length; ++i) {
    if (userStore.profile?.user.resources[i].restypeId === 2) {
...
async function deleteBotsFunction() {
  const data = new FormData();
